    ...
    if (this.$error) {
      if (this.nestedKeys.length > 0) {
        return buildFromKeys(this.nestedKeys, function (key) {
          return _this5.refProxy(key).$messages;
        });
      } else if (this.ruleKeys.length > 0) {
        return this.ruleKeys.reduce(function (errors, key) {
          if (!_this5.getRef(key).proxy && !_this5.getRef(key).$pending) {
            errors.push(_this5.getRef(key).$messages);
          }
          return errors;
        }, []);
      }
  ...
  var ValidationRule = VBase.extend({
    ...
    computed: {
      ...
      $messages: function $messages() {
        if (!this.proxy) {
          if (this.run.params) {
            var tmpl = this.rootModel._templates || _templates2.default;
    ...
    computed: _extends({}, validationGetters, {
      ...
      children: function children() {
        ...
        return [].concat(this.nestedKeys.map(function (key) {
          return renderNested(_this8, key);
        }), this.ruleKeys.map(function (key) {
          return renderRule(_this8, key);
        })).filter(Boolean);
      }
    })
  });
    ...
    computed: {
      ...
      tracker: function tracker() {
        ...
        return trackBy ? function (key) {
          return '' + getPath(_this9.rootModel, _this9.getModelKey(key), trackBy);
        } : function (x) {
          return '' + x;
        };
      },
      ...
      children: function children() {
        ...
        return this.keys.map(function (key) {
          ...
          return (0, _vval.h)(Validation, track, {
            ...
            prop: key,
            lazyParentModel: _this10.eagerParentModel,
            model: model[key],
            rootModel: _this10.rootModel
          });
        }).filter(Boolean);
      }
    },
...
var validationMixin = {
  ...
  beforeCreate: function beforeCreate() {
    ...
    options.computed.$v = function () {
      return _this11._vuelidate.refs.$v.proxy;
    };
  },
  beforeDestroy: function beforeDestroy() {
    if (this._vuelidate) {
